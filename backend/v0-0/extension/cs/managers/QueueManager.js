...
export default class QueueManager {
    ...
    enqueueWithMetadata(url) {
        try {
            ...
            logger.debug("enqueueWithMetadata failed", e);
        }
    }

    getFirstQueueItem() {
        try {
            return this.queue && this.queue.length > 0 ? this.queue[0] : null;
        } catch {
            return null;
        }
    }
    ...
    setupDragAndDrop() {
        const sharetube_main = this.app.sharetube_main;
        const onEnter = (e) => {
            e.preventDefault();
            e.stopPropagation();
            sharetube_main.classList.add("dragover");
        };
        const onOver = (e) => {
            e.preventDefault();
        ...
        const onLeave = (e) => {
            e.preventDefault();
            sharetube_main.classList.remove("dragover");
        };
        const onDrop = (e) => {
            e.preventDefault();
            e.stopPropagation();
        ...
        sharetube_main.addEventListener("dragenter", onEnter);
        sharetube_main.addEventListener("dragover", onOver);
        sharetube_main.addEventListener("dragleave", onLeave);
        sharetube_main.addEventListener("drop", onDrop);
    }
    ...
    async removeQueueItem(comp) {
        try {
            ...
            logger.debug("removeQueueItem failed", e);
        }
    }
    ...
    async sendQueueAdd(code, item) {
        try {
            ...
            logger.debug("sendQueueAdd failed", e);
        }
    }
    ...
    onQueueSnapshot(payload) {
        try {
            ...
            if (items.length > 0) {
                ...
                const changed =
                    dur !== this.app.playback.duration ||
                    prog !== this.app.playback.progress ||
                    ps !== this.app.playback.playing_since;
                if (changed) {
            ...
            try {
                ...
                if ((stateNow === "starting" || stateNow === "playing") && code) {
                    if (!this.app.roomManager._ensureRoomHashApplied(code))
                        this.app.roomManager._navigateToActiveVideo(code);
                }
            } catch {}
        } catch (e) {
            logger.debug("onQueueSnapshot failed", e);
        }
    }
}
