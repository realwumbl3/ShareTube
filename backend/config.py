...
class Config:
    ...
    # Default SQLAlchemy URI pointing to a sqlite database stored in instance/newapp.db
    _DB_DEFAULT = f"sqlite:///{os.path.join(_ROOT, 'instance', 'newapp.db')}"
    ...
    ACCESS_TOKEN_EXPIRES = int(
        os.getenv("ACCESS_TOKEN_EXPIRES_SECONDS", str(int(timedelta(days=14).total_seconds())))
    )
