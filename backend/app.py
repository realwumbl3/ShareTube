...
logging.basicConfig(level=logging.INFO, format=log_format, filename="instance/app.log")
...
def create_app() -> Flask:
    ...
    try:
        # Controls API blueprint for room control endpoints
        from .views.controls import controls_bp

        app.register_blueprint(controls_bp)
    except Exception:
        logging.exception("controls blueprint import failed")
    # Register sockets (module import binds handlers)
    try:
        # Importing handlers attaches event listeners to the socketio instance
        from .sockets import handlers  # noqa: F401
    except Exception:
        logging.exception("socket handlers import failed")
