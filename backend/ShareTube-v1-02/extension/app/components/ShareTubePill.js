    ...
    constructor(app) {
        ...
        html`
            <div id="sharetube_pill" is_locked=${this.pillLocked.interp()}>
                <button zyx-if=${this.pillLocked} id="sharetube_lock_btn" class="lock_btn" zyx-click=${() => this.setLock(false)}>
                ...
                />
                <div zyx-else class="sign_in_button rounded_btn" zyx-click=${() => this.app.authManager.openSignInWithGooglePopup()}>
                    Sign in with <img src=${googleSVG} alt="Google" />
                </div>
                ${this.logo}
                <span zyx-if=${state.userId}>
                    ${this.userIcons}
                    <div zyx-if=${state.roomCode} id="sharetube_toggle_queue" class="rounded_btn" zyx-click=${() => this.app.queue.toggleQueueVisibility()}>
                        ${state.queueQueued.interp((v) => (v.length > 0 ? `Queue (${v.length})` : "Queue empty."))}
                    </div>
                    ${this.controls}
                </span>
                <button zyx-if=${state.debug_mode} class="rounded_btn" zyx-click=${() => this.app.debugMenu.toggleVisibility()}>dbg</button>
            </div>
        `.bind(this);
