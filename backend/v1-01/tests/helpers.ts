...
const PROFILE_A = path.join(process.cwd(), ".browser-profiles", "A");
const PROFILE_B = path.join(process.cwd(), ".browser-profiles", "B");
const EXTENSION_PATH = path.join(process.cwd(), "extension", "v1");
...
export async function withExtension(run: (page: any, context: any) => Promise<void>) {
    ...
    const context = await launchExtensionContext(PROFILE_A, {
        headless: false,
