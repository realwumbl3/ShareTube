    ...
    def _on_user_ready(room: Room, user_id: int, data: dict):
        ...
        try:
            ...
            membership = (
                ...
                .filter_by(room_id=room.id, user_id=user_id, active=True)
                .first()
            )
            if not membership:
                logging.warning(
                    "user.ready: membership not found (room=%s, user=%s)",
                    room.code,
                    user_id,
                )
                return
            ...
            logging.debug(
                "user.ready: before update membership.ready=%s room.state=%s current_entry_id=%s",
                membership.ready,
                room.state,
                getattr(getattr(room.current_queue, "current_entry", None), "id", None),
            )
            membership.set_ready(ready)
            db.session.flush()
            logging.debug(
                "user.ready: after update membership.ready=%s ready=%s room.state=%s",
                membership.ready,
                ready,
                room.state,
            )
