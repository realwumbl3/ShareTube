    ...
    onRoomPresence(payload) {
        try {
            ...
            const incomingIds = new Set(
                members
                    .map(function (m) {
                        return m && m.id != null ? Number(m.id) : null;
                    })
                    .filter(function (v) {
                        return v != null;
                    })
            ...
            for (const [uid] of Array.from(this.app.presentUsersById.entries())) {
                if (!incomingIds.has(Number(uid))) {
                    ...
                    try {
                        this.app.adUserIds.delete(Number(uid));
                    } catch (e) {
                        logger.debug("adUserIds delete failed", e);
                    }
                }
            }
            if (this.app.justJoinedCode && this.app.justJoinedCode === payload.code) {
                try {
                    ...
                    logger.debug("presence log failed", e);
                }
            ...
            logger.debug("onRoomPresence failed", e);
        }
    }
}
