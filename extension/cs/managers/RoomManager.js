...
export default class RoomManager {
	...
	get roomState() { return this.app.roomState; }

	// --- Public helpers for other managers ---
	isOnVideoUrl(url) { return this._isOnVideoUrl(url); }
	buildUrlWithRoomHash(url, code) { return this._buildUrlWithRoomHash(url, code); }
	...
	async togglePlayPause() {
		...
		if (next === 'idle') {
			try { if (this.app.persistProgress) this.app.persistProgress(); } catch { }
		}
	...
	onControlButtonClicked() {
		try {
			...
			const inAd = rs === 'playing_ad' || (this.app.adPlaying && this.app.adPlaying.get && this.app.adPlaying.get()) || (this.app.adUserIds && this.app.adUserIds.size > 0);
			if (inAd) {
	...
	onRoomStateChange(payload) {
		try {
			...
			try { if (this.app.updateControlButtonLabel) this.app.updateControlButtonLabel(); } catch { }
	...
	_shouldNavigateToActiveVideo(initial, next) {
		try { return (initial === 'idle' && next === 'starting') || (initial === 'starting' && next === 'playing') || (initial === 'idle' && next === 'playing'); } catch { return false; }
	}
	...
	_buildUrlWithRoomHash(url, code) {
		try { const u = new URL(url, location.href); u.hash = "sharetube:" + String(code); return u.toString(); } catch { return String(url) + "#sharetube:" + String(code); }
	}
	...
	async handlePlusButton() {
		...
		if (code) {
			try { copyWatchroomUrl(code); } catch { }
			return;
		}
		const sock = await this.app.socketManager.ensureSocket();
		if (!sock) {
			try { console.warn("No socket/auth; cannot create room"); } catch { }
			return;
		}
	...
	onRoomCreateResult(res) {
		...
		if (!res || !res.ok) {
			try { console.warn("room_create failed", res); } catch { }
			return;
		}
		const code = res.code;
	...
	onRoomJoinResult(res) {
		...
		if (!res || !res.ok) {
			try { console.warn("room_join failed", res); } catch { }
			return;
		}
		const code = res.code;
		this.app.roomCode.set(code || "");
	...
	async tryJoinFromUrlHash() {
		try {
			...
			if (!m) return;
			const code = m[1];
			this.app._withSocket(function (sock) { sock.emit("room_join", { code: code }); }, code);
			this.app.roomCode.set(code);
	...
	onRoomAdStatus(payload) {
		try {
			...
			this.app.adUserIds = new Set(ids.map(function (x) { return Number(x); }));
			try { if (this.app.adOverlayManager) this.app.adOverlayManager.notifyStateChanged(); } catch { }
	...
	updatePlaybackEnforcement(reason) {
		try {
			const rs = this.roomState.get();
			const localInAd = this.app._isInAd ? this.app._isInAd() : false;
			...
			if (this.app.player) {
				...
				if (!shouldPlay && !localInAd) {
					try { this.app.player.requestPause(); } catch { }
					if (rs === 'starting' && this.app.player.video) {
						const posMs = this.getServerSuggestedPositionMs();
						const nearZero = posMs <= 1500 && (Number(this.app.playback && this.app.playback.progress || 0) <= 1000) && (Number(this.app.playback && this.app.playback.playing_since || 0) <= 0);
						if (nearZero && this.app.player.video.currentTime > 0.05) {
							try { this.app.player.video.currentTime = 0; } catch (e) { logger.debug('set currentTime 0 failed', e); }
						}
					}
				} else if (shouldPlay) {
					try { this.app.player.requestPlay(); } catch (e) { logger.debug('requestPlay failed', e); }
				}
	...
	updateControlButtonLabel() {
		try {
			...
			const inAd = s === 'playing_ad' || (this.app.adPlaying && this.app.adPlaying.get && this.app.adPlaying.get()) || (this.app.adUserIds && this.app.adUserIds.size > 0);
			btn.textContent = inAd ? 'Playing AD' : (s === 'playing' ? 'Pause' : 'Play');
		} catch { }
	}

	shouldPlayContent(roomState, localInAd, anyoneInAds) {
		try { return (roomState === 'playing') && !localInAd && !anyoneInAds; } catch { return false; }
	}
	...
	getServerSuggestedPositionMs() {
		...
		const duration = pb.duration, progress = pb.progress, playing_since = pb.playing_since;
		...
		try {
			...
			if (this.app.player && this.app.player.video) {
				...
				if (isFinite(posSec) && diff > 0.25) {
					try { if (this.app.player.suppressSeekEmit) this.app.player.suppressSeekEmit(800); } catch { }
	...
	onRoomSeek(payload) {
		try {
			...
			this.app.playback = { duration: dur, progress: Math.max(0, ms), playing_since: play ? Date.now() : 0, lastTs: Date.now() };
			try { if (this.app.player && this.app.player.suppressSeekEmit) this.app.player.suppressSeekEmit(800); } catch { }
